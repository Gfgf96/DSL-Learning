    ...
    try:
        while True:
            ...
            if message_type == "start_recording":
                ...
                # Extract landmarks
                landmarks = await asyncio.to_thread(hand_capture.extract_landmarks, frame)
                ...
                if is_dynamic:
                    ...
                    if dynamic_predictor:
                        ...
                        if is_ready:
                            ...
                            try:
                                pred_result = await asyncio.to_thread(dynamic_predictor.predict)
                            except Exception as e:
                    ...
                    if static_predictor:
                        try:
                            pred_result = await asyncio.to_thread(static_predictor.predict, norm_landmarks)
                        except Exception as e:
...
if FRONTEND_OUT_DIR.exists() and FRONTEND_OUT_DIR.is_dir():
    app.mount("/", StaticFiles(directory=str(FRONTEND_OUT_DIR), html=True), name="static_out")
elif FRONTEND_OLD_DIR.exists() and FRONTEND_OLD_DIR.is_dir():
    # Fallback to old frontend behavior
    app.mount("/static", StaticFiles(directory=str(FRONTEND_OLD_DIR)), name="static_old")
