...
from src.backend.core.config import STATIC_LETTERS, ALL_LETTERS
...
class LetterSequence:
    ...
    def __init__(self, mode: str = "sequential", include_dynamic: bool = False):
        ...
        self.available_letters = ALL_LETTERS if include_dynamic else STATIC_LETTERS
    ...
    def _get_random(self, current_letter: Optional[str]) -> str:
        ...
        print(
            f"🎲 Random mode: selected '{choice}' from {len(candidates)} candidates (avoiding '{current_letter}')"
        )
        return choice
