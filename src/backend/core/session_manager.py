...
class UserSession:
    ...
    def add_prediction(
        self, predicted_letter: str, confidence: float
    ) -> Optional[Dict]:
        """
    ...
    def finish_recording(self) -> Dict:
        ...
        valid_predictions = [
            p
            for p in self.recording_predictions
        ...
        print(
            f" Predictions: {dict(letter_counts)} | Winner: {most_common_letter} ({count}/{len(valid_predictions)})"
        )
        ...
        if self.mode == "sentence":
            if self.target_sentence and self.current_letter:
                ...
                if match:
                    return self._handle_sentence_success(
                        most_common_letter, avg_confidence
                    )
                else:
                ...
                return self._handle_free_mode_success(
                    most_common_letter, avg_confidence
                )
        else:
            match = most_common_letter == self.current_letter
    ...
    def _handle_success(self, predicted_letter: str, confidence: float) -> Dict:
        ...
        print(
            f" Success! Correctly signed '{self.current_letter}' (confidence: {confidence:.2f})"
        )
    ...
    def _handle_sentence_success(
        ...
        print(
            f" Success! Correctly signed '{predicted_letter}' (confidence: {confidence:.2f})"
        )
...
class SessionManager:
    ...
    def create_session(
        self, session_id: Optional[str] = None, mode: str = "sequential"
    ) -> UserSession:
