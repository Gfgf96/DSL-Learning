...
class LandmarkRecorder:
    def __init__(self):
        ...
        # Ensure MediaPipe `.task` file is present for the recorder. If it's missing,
        # attempt to download the official mediapipe release artifact used for
        # the hand landmarker model. This keeps the download out of the web app
        # cold-start path and only fetches the task file when data collection
        # tooling is used.
        if not MODEL_PATH.exists():
            MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
            mp_url = "https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/latest/hand_landmarker.task"
            try:
                print(f"Downloading MediaPipe task file to {MODEL_PATH}...")
                resp = requests.get(mp_url, stream=True, timeout=30)
                ...
                print(f"Failed to download hand_landmarker.task: {e}")
                print(f"Please download manually and place it at {MODEL_PATH}")
    ...
    def run(self):
        ...
        while True:
            ...
            cv2.putText(
                frame,
                f"Count: {len(self.data[current_letter])}",
                (10, 70),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.8,
                (0, 255, 255),
            ...
            cv2.putText(frame, status, (300, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            ...
            for i, letter in enumerate(LETTERS):
                ...
                cv2.putText(
                    frame, txt, (10 + i * 150, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, clr, 2
                )
            ...
            if key == ord("q"):
                ...
                if hand_detected:
                    ...
                    if time.time() - self.last_save_time > 0.2:
                        ...
                        print(
                            f"[OK] {current_letter}: {len(self.data[current_letter])}"
                        )
            elif key == ord("1"):
                self.current_letter_idx = 0
            elif key == ord("2"):
