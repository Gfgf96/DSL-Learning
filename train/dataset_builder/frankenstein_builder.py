        ...
        for letter, source in LETTER_SOURCE.items():
            ...
            if source == "asl":
                ...
                if len(landmarks) == 0:
                    ...
                    print("   [WARNING] DGS empty, fallback to ASL (MAY CAUSE ERRORS!)")
                    landmarks = self.extract_asl_landmarks(
                        letter, self.samples_per_class
                    )
...
if __name__ == "__main__":
    # hand_landmarker.task is downloaded automatically via downloader script
    # or manually via:
